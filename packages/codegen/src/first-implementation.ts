...
const firstImplementation = generateCodeGoal.register({
...
firstImplementation.define(async ({ simulationDescription }, { model }) => {
  ...
  try {
    const result = await generateObject({
      ...
      temperature: 0.1,
      system: systemPrompt,
      prompt: `Create a SimuLLM simulation for: ${simulationDescription}`,
      schema: z.object({
...
const _ = `Here is the complete simullm library they can use:
